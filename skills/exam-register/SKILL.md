---
name: exam-register
description: Visually extract exam PDF questions and register editable text, preserved source images, and interactive simulations through TeamsWord MCP. Use for exam registration, simulation item authoring, source comparison, and resuming recorded registrations.
metadata:
  version: "1.1.0"
---
...
- 답이 보기 중 하나도 글자 하나도 아니고 **조작 결과의 상태**이면 시뮬레이션 문항이다. 절차와 계약 요건은 [시뮬레이션 규칙](references/simulation-rules.md)을 따른다. 원본에 없는 조작을 만들어 내지 않는다 — 사용자가 요청했거나 원본이 조작 결과 자체를 답으로 요구할 때만이다.
...
5. `scripts/check_manifest.py <question.json> --workspace <작업폴더>`를 실행하고 원본과 전사·크롭을 시각 대조한다. 스크립트 통과를 내용 정확성으로 보고하지 않는다. 시뮬레이션 문항은 `scripts/check_simulation.py <question.json> --workspace <작업폴더>`도 실행한다 — 자산 HTML 의 형식·자족성·규약 문자열과 config·initial·정답의 앞뒤를 본다. 이 통과도 계약 적합성이 아니다(요건 1·2·4·9·12 는 돌려 봐야 안다).
...
7. 저장 내용을 MCP로 재조회하고 실제 편집기 화면을 원본과 비교한다. 화면 캡처는 `runs/<id>/verification/`에 PNG로 저장해 visual 증거로 기록한다(파일이 없으면 passed 불가). [검증 기준](references/verification.md)에 따라 등록·구조·화면·정답·배점 상태를 분리한다.
...
- 사례: [이미지 자료](examples/image-stimulus/decision.md), [텍스트 보기](examples/text-viewbox/decision.md), [연표](examples/timeline-image/decision.md), [복합 편지](examples/composite-letter/decision.md), [지레 균형 시뮬레이션](examples/simulation-balance/decision.md).
